...
def uploadVideo(session_id, video, title, tags, schedule_time=0, verbose=True):
    if (
        schedule_time - datetime.datetime.now().timestamp() > 864000
    ):  # 864000s = 10 days
    ...
    if r.json()["status_code"] == 0:
        if verbose:
            print(
                f"[+] Video upload uploaded successfully {'| Scheduled for '+str(schedule_time) if schedule_time else ''}"
            )
    else:
        printError(url, r)
        return False
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-i", "--session_id", help="Tiktok sessionid cookie", required=True
    )
    ...
    parser.add_argument(
        "--tags", nargs="*", default=[], help="List of hashtags for the video"
    )
    parser.add_argument(
        "-s",
        "--schedule_time",
        type=int,
        default=0,
        help="schedule timestamp for video upload",
    )
